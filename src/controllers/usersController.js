...
const userController = {
  getUser: async (req, res) => {
    try {
      ...
      const users = await User.find({ ...condition, role: "R1" }, option).sort(
        sort
      );
      ...
      if (!_.isEmpty(users)) {
        ...
        return res
          .status(200)
          .json({ status: 2, message: "Cannot find users!" });
      }
    } catch (err) {
  ...
  checkUserExists: async (req, res) => {
    try {
      ...
      if (!_.isEmpty(user)) {
        ...
        return res
          .status(200)
          .json({ status: 2, message: "User dosen't exists!" });
      }
    } catch (err) {
      console.log("Error: " + err);
      return res.status(400).json({ status: 1, err });
  ...
  addUser: async (req, res) => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        return res
          .status(200)
          .json({ status: 0, message: "Insert user successfully!" });
      } else {
        return res
          .status(200)
          .json({
  ...
  updateUser: async (req, res) => {
    try {
      ...
      if (result.modifiedCount == 0)
        return res
          .status(200)
          .json({ status: 2, message: "Update user failed!" });

      return res
        .status(200)
        .json({ status: 0, message: "Update user successfully!" });
    } catch (err) {
      console.log("Error: " + err);
      return res.status(400).json({ status: 1, err });
    }
  ...
  deleteUser: async (req, res) => {
    try {
      ...
      if (result.deletedCount == 0)
        return res
          .status(200)
          .json({ status: 3, message: "Delete user failed!" });

      return res
        .status(200)
        .json({ status: 0, message: "Delete user successfully!" });
    } catch (err) {
      console.log("Error: " + err);
      return res.status(400).json({ status: 1, err });
    }
